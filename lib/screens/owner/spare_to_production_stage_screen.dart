...
class SpareToProductionStageScreen extends StatefulWidget {
  final Map<String, dynamic> spare;

  const SpareToProductionStageScreen({super.key, required this.spare});

  @override
  State<SpareToProductionStageScreen> createState() => _SpareToProductionStageScreenState();
}

class _SpareToProductionStageScreenState extends State<SpareToProductionStageScreen> {
  final _extractionCompletedDateCtrl = TextEditingController();
  final _productionDateCtrl = TextEditingController();
  ...
  @override
  void dispose() {
    _extractionCompletedDateCtrl.dispose();
    _productionDateCtrl.dispose();
    _expectedProductionDateCtrl.dispose();
    super.dispose();
  }

  Future<void> _submit() async {
    if (_extractionCompletedDateCtrl.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Extraction Completed Date is required')));
      return;
    }
    if (_productionDateCtrl.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Production Date is required')));
      return;
    }
    ...
    try {
      final payload = {
        'extractionCompletedDate': _extractionCompletedDateCtrl.text,
        'productionDate': _productionDateCtrl.text,
        ...
        : SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (spare['extractionSentDate'] != null)
                          _buildInfoRow('Extraction Sent', spare['extractionSentDate']),
                        if (spare['expectedExtractionDate'] != null)
                          _buildInfoRow('Expected Extraction', spare['expectedExtractionDate']),
                      ],
                    ),
                  ),
                ),
                ...
                TextField(
                  controller: _extractionCompletedDateCtrl,
                  readOnly: true,
                  onTap: () => _selectDate(_extractionCompletedDateCtrl),
                  decoration: const InputDecoration(
                    labelText: 'Extraction Completed Date',
                ...
                const SizedBox(height: 16),
                TextField(
                  controller: _productionDateCtrl,
                  readOnly: true,
                  onTap: () => _selectDate(_productionDateCtrl),
                  decoration: const InputDecoration(
