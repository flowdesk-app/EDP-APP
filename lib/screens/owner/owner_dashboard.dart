  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        _productionJobs = jobs.where((j) => j.status != 'Removed' && j.status != 'Closed' && j.status != 'Delivered' && j.status != 'Returned' && j.status != 'Completed' && !(j.jobType == 'New' && j.status == 'Blank Order') && !(j.jobType == 'Re-coating' && (j.status == 'Created' || j.status == 'Arrived' || j.status == 'Extracted'))).length;
