...
class _ReadyForDeliveryScreenState extends State<ReadyForDeliveryScreen> {
  ...
  Future<void> _refreshJobs() async {
    ...
    try {
      ...
      final freshJobs = allJobs.where((j) => j.status == 'Completed').toList();
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(_isEditing ? '${_selectedJobIds.length} Selected' : 'Ready for Delivery', style: const TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF202124))),
